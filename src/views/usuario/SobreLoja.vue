<template>
    <v-container class="pa-0">
        ...
        <div class="simple-tabs">
            <div class="tab" :class="{ active: tab === 'sobre' }" @click="tab = 'sobre'">
                Sobre
            </div>

            <div class="tab" :class="{ active: tab === 'horario' }" @click="tab = 'horario'">
                Horário
            </div>
            ...
            <v-window v-model="tab">
                ...
                <v-window-item value="sobre">
                    ...
                    <div class="content-center">
                        ...
                        <div class="buttons">
                            <v-btn v-if="lojista?.whatsapp" variant="outlined" prepend-icon="mdi-whatsapp"
                                :href="`https://wa.me/${lojista.whatsapp}`" target="_blank">
                                {{ lojista.whatsapp }}
                            </v-btn>
                        </div>
                ...
                <v-window-item value="horario">
                    <v-list>
                        <v-list-item v-for="dia in horarios" :key="dia.nome" :class="{ 'dia-atual': dia.isHoje }">
                            <v-list-item-title>
                                {{ dia.nome }}
                            </v-list-item-title>
                        ...
                        <v-col v-for="(ativo, metodo) in lojista?.formasPagamento" :key="metodo" cols="6">
                            <v-card class="payment-card" variant="outlined">
                                <v-card-text class="d-flex align-center">
                                    <v-icon class="mr-2">
                                        {{
                                            metodo === 'pix' ? 'mdi-qrcode'
                                                : metodo === 'cartaoCredito' ? 'mdi-credit-card'
                                                    : metodo === 'cartaoDebito' ? 'mdi-credit-card-outline'
                                                        : 'mdi-cash'
                                        }}
                                    </v-icon>
                                    ...
                                    <span>
                                        {{
                                            metodo === 'cartaoCredito' ? 'Crédito'
                                                : metodo === 'cartaoDebito' ? 'Débito'
...
export default {
    ...
    async mounted() {
        this.lojista = await this.getLojista(this.$route.query.id as string)
    ...
    methods: {
        ...
        montarHorarios() {
            ...
            this.horarios = mapaDias.map(dia => {
                const horario = this.lojista?.horarioFuncionamento?.[dia.key as keyof typeof this.lojista.horarioFuncionamento];
