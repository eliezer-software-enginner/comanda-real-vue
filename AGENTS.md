...
This file contains guidelines and commands for agentic coding agents working in this Vue.js + TypeScript + Firebase repository.
...
- Test files use `.test.ts` extension alongside service files

### Code Quality Commands
- `npm run lint` - Run ESLint with auto-fix and caching
- `npm run type-check` - Run TypeScript type checking
- `npm run format` - Format code with Prettier
- `npm run pre-commit` - Run build + unit tests (pre-commit hook)
...
- Use generic types properly (`CrudService<InputData, T extends Identificavel>`)
...
- Use CSS Modules with BEM-like naming: `.formGroup`, `.saveButton`
...
- **IMPORTANT**: Always commit immediately after implementing working changes
